...
import { Scale, FileText, Boxes, Truck } from 'lucide-react';

export default function Dashboard() {
    const { user, profile } = useAuth();

    const roleLabel = profile?.role === 'admin' ? 'Administrador' : profile?.role || 'Sin asignar';
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-500">
                ...
                {[
                    ...
                    >
                        <div className="flex items-center justify-between">
                            ...
                            <card.icon size={18} className="text-slate-400 dark:text-slate-600" />
                        </div>
                        <div className="text-2xl font-bold text-slate-900 dark:text-white">
                            —
                        </div>
                        <p className="text-xs text-slate-400 dark:text-slate-600">Sin datos aún</p>
                    </div>
                ))}
            </div>
            ...
            <div className="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 p-10 text-center">
                <Scale size={40} className="mx-auto text-slate-300 dark:text-slate-700 mb-4" />
                <p className="text-slate-500 dark:text-slate-400 text-sm font-medium">
                    Las gráficas de operación aparecerán aquí conforme se registren tickets y pesajes.
                </p>
