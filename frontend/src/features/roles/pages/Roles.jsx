  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
              Estos perfiles tienen acceso completo a todo el sistema sin
              restricciones, independientemente de los permisos definidos abajo.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        {SYSTEM_LABELS.map(({ label, title }) => {
          ...
          return (
            <div
              key={label}
              className={`rounded-xl border ${style.border} bg-white dark:bg-slate-900 overflow-hidden`}
            >
              {/* Header del perfil */}
              <button
              ...
              >
                <div className="flex items-center gap-4">
                  <div
                    className={`h-10 w-10 rounded-full ${style.bg} flex items-center justify-center shrink-0`}
                  >
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-0.5">
                      ...
                      <span
                        className={`px-2 py-0.5 rounded-full text-[11px] font-mono font-semibold ${style.badge}`}
                      >
                        {label}
                      </span>
                    </div>
      ...
      <p className="text-xs text-slate-400 dark:text-slate-600 text-center">
        Los permisos están definidos por perfil. El acceso real en Appwrite lo
        controlan los labels de cada usuario.
      </p>
    </div>
  );
}
