...
import { Users2, Plus, Search, Filter, Pencil, PowerOff, Power, ChevronRight, Loader2, ShieldAlert } from 'lucide-react';
...
export default function Usuarios() {
    ...
    if (!can('users.view')) {
        return (
            <div className="flex flex-col items-center justify-center py-24 text-center">
                ...
                <p className="text-slate-500 dark:text-slate-400 text-sm">No tienes permiso para ver este módulo.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
                                    ...
                                    return (
                                        <tr key={u.$id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50">
                                            ...
                                            <td className="px-4 sm:px-6 py-4">
                                                ...
                                                {u.employeeCode && (
                                                    <div className="text-xs text-slate-400 mt-0.5">{u.employeeCode}</div>
                                                )}
                                                {/* email visible en mobile */}
                                                <div className="text-xs text-slate-400 mt-0.5 md:hidden">{u.email || '—'}</div>
                                            </td>
                                            {/* Email */}
            ...
            <Dialog.Root open={Boolean(confirmDisable)} onOpenChange={(v) => { if (!v) setConfirmDisable(null); }}>
                <Dialog.Portal>
                    ...
                    <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-full max-w-sm mx-4 bg-white dark:bg-slate-900 rounded-xl shadow-xl p-6">
                        ...
                        <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">
                            ¿Confirmas que deseas deshabilitar a <strong className="text-slate-700 dark:text-slate-200">{confirmDisable ? displayName(confirmDisable) : ''}</strong>?
                    ...
                    <div className="flex-1 overflow-y-auto p-6 space-y-6">
                        ...
                        {u.notes && (
                            <DetailSection title="Notas internas">
                                <p className="text-sm text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{u.notes}</p>
                            </DetailSection>
                        )}
                        ...
                        <DetailSection title="Sistema">
                            ...
                            <DetailRow
                                label="Creado el"
                                value={u.$createdAt ? new Date(u.$createdAt).toLocaleDateString('es-MX', { dateStyle: 'medium' }) : null}
                            />
                        </DetailSection>
                    </div>
                </Dialog.Content>
...
function DetailRow({ label, value, mono = false }) {
    return (
        <div className="flex justify-between gap-4">
            ...
            <span className={`text-sm text-slate-900 dark:text-slate-100 text-right ${mono ? 'font-mono text-xs' : ''}`}>
